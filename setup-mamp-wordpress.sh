...
echo "Setting up WordPress with MAMP..."

# Create database using MAMP's MySQL
/Applications/MAMP/Library/bin/mysql -u root -proot << EOF
CREATE DATABASE IF NOT EXISTS skerritt_wp;
CREATE USER IF NOT EXISTS 'wordpress'@'localhost' IDENTIFIED BY 'wordpress';
GRANT ALL PRIVILEGES ON skerritt_wp.* TO 'wordpress'@'localhost';
FLUSH PRIVILEGES;
EOF

echo "✅ Database created"

# Update wp-config.php for MAMP
...
// ** Database settings - MAMP specific ** //
...
echo "✅ wp-config.php configured for MAMP"

# Import content
/Applications/MAMP/Library/bin/mysql -u wordpress -pwordpress skerritt_wp < wordpress-content-import-fixed.sql
...
echo "Next steps:"
echo "1. Open MAMP"
echo "2. Click 'Preferences' → 'Web Server'"
echo "3. Change Document Root to: /Users/chrisskerritt/SEC"
echo "4. Click OK and Start Servers"
echo "5. Visit http://localhost:8888 to complete WordPress setup"